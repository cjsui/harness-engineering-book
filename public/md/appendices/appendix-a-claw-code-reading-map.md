...
> 如果你時間有限，只想花 30 分鐘建立對 `claw-code` 的整體印象，請依序閱讀以下四個檔案：
>
> | 順序 | 檔案 | 花多少時間 | 你會得到什麼 |
> |---|---|---|---|
> | 1 | `claw-code/README.md` | 5 分鐘 | 知道這個系統是什麼、為什麼存在 |
> | 2 | `claw-code/USAGE.md` | 5 分鐘 | 知道使用者怎麼操作這個系統 |
> | 3 | `claw-code/rust/crates/runtime/src/lib.rs` | 10 分鐘 | 看到 runtime 的主要型別和公開介面 |
> | 4 | `claw-code/rust/crates/tools/src/lib.rs` | 10 分鐘 | 理解工具系統的宣告與分發機制 |
>
> 完成這四個檔案後，你對 `claw-code` 就不再是一片空白，可以再根據興趣深入。
...
| 本書章節 | 你要回答的問題 | 優先閱讀檔案 | 第一輪先看什麼 | 初學者建議先看 |
|---|---|---|---|---|
| 第 1 章 | 為什麼這不是單純 prompt 工具？ | `README.md`, `PHILOSOPHY.md` | 產品定位、agent 工作方式、設計哲學 | ✅ 建議先看 |
| 第 4 章 | 如何建立 reading strategy？ | `README.md`, `USAGE.md`, `rust/README.md` | 入口文件、workspace layout、crate responsibilities | ✅ 建議先看 |
| 第 5 章 | 系統從哪裡進來？ | `USAGE.md`, `rusty-claude-cli` 相關檔案 | subcommands、modes、system surface | ✅ 建議先看 |
| 第 6 章 | runtime loop 怎麼運作？ | `runtime/src/lib.rs`, `runtime/src/conversation.rs` | `ApiRequest`、`AssistantEvent`、turn loop | ✅ 建議先看 |
| 第 7 章 | tools 怎麼被定義與暴露？ | `tools/src/lib.rs` | `ToolSpec`、registry、`definitions()` | ✅ 建議先看 |
| 第 8 章 | guardrails 在哪裡真的生效？ | `runtime/src/permissions.rs` 及相關 enforcer 檔案 | `PermissionMode`、policy、授權判斷 | ✅ 建議先看 |
| 第 9 章 | session 與 transcript 如何持久化？ | `runtime/src/session.rs` | append、resume、transcript 結構 | ⏭️ 可稍後看 |
| 第 10 章 | system prompt 怎麼被組起來？ | `runtime/src/prompt.rs` | project context、instruction files、prompt assembly | ⏭️ 可稍後看 |
| 第 11 章 | config 與 modes 怎麼影響行為？ | `runtime/src/lib.rs` 與 config 相關檔案 | hierarchy、settings、環境敏感行為 | ⏭️ 可稍後看 |
| 第 12 章 | MCP / plugins 到底做到哪裡？ | extension / bridge / registry 相關檔案 | implemented path 與 planned surface 的邊界 | ⏭️ 可稍後看 |
| 第 13 章 | remote 為什麼是另一種問題？ | remote / proxy / execution context 相關檔案 | local vs remote boundary | ⏭️ 可稍後看 |
| 第 14 章 | 系統怎麼驗證自己沒退化？ | `tests/mock_parity_harness.rs`, `mock_parity_scenarios.json` | scenario-based verification | ⏭️ 可稍後看 |
| 第 15 章 | 如何把整體重新接回來？ | `rust/README.md`, `runtime/src/lib.rs`, `tools/src/lib.rs` | end-to-end 系統鏈 | ⏭️ 可稍後看 |
