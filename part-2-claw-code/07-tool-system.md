...
> 
> 讀完本章，你將會學到：
> - 每個工具都有正式的「身分證」（`ToolSpec`），不只是一段程式碼
> - 所有工具被集中管理在 `GlobalToolRegistry`，不是零散散落各處
> - 模型看到的工具清單是經過篩選的，不是系統所有工具的全貌
> - `--allowedTools` 搭配別名正規化，讓使用者方便限制工具範圍
> - built-in、runtime、plugin 三種工具來源最終統一輸出給模型
...
- [ ] 理解第 6 章的 `ConversationRuntime` 與 `ToolExecutor` 的角色
- [ ] 知道什麼是 JSON Schema（用來描述資料結構的格式規範）
- [ ] 了解「權限（permission）」的基本概念——不是所有人都能做所有事
- [ ] 知道什麼是 registry / catalog（統一登記與查找的管理機制）
- [ ] 理解「別名（alias）」的概念——同一個東西可以有不同的稱呼
...
> 💡 **生活化比喻**：`ToolSpec` 就像每個員工的「工作證」。工作證上不只有名字（name），還有職位說明（description）、負責的業務範圍（input_schema），以及這個職位需要什麼級別的門禁卡才能進出特定區域（required_permission）。沒有工作證的員工就只是「某個人」，有了工作證，公司才能統一管理誰能做什麼、誰有權進哪裡。
...
> 💡 **生活化比喻**：`GlobalToolRegistry` 就像一間大學的「課程登錄系統」。每個學期有固定必修課（built-in tools）、有教授臨時加開的選修課（runtime tools）、還有和外校合作的交換課程（plugin tools）。登錄系統會自動檢查有沒有重複的課號、衝堂的時段，確保最後公布出來的選課清單不會讓人選到兩門一模一樣的課。
...
> 💡 **生活化比喻**：registry 和 dispatch 的關係就像「餐廳菜單」和「廚房出餐」。菜單（registry）告訴你有什麼菜可以點、每道菜的材料和價格是什麼；但當你真的點了一道菜，廚房（dispatch）才會開始做菜並端上桌。菜單負責「有什麼可選」，廚房負責「怎麼做出來」。如果把兩者混在一起，你就沒辦法在不動廚房的情況下單獨調整菜單。

> ⚠️ **初學者常見誤區**：很多人覺得「把工具接上去」就是在一個 map 裡放 `tool_name → function` 就好。但這樣做會讓你的系統缺少 schema（模型不知道怎麼用）、缺少 permission（任何工具都能被呼叫）、缺少來源管理（不知道工具從哪來）。真正的 tool system 需要宣告、治理、過濾三者並存。
...
> 💡 **生活化比喻**：這就像便利商店結帳時，你說「我要一杯美式」，店員知道你指的是「冰美式咖啡（中杯）」。你說的是日常用語（alias），店員內部系統登記的是正式品名（canonical name）。`normalize_allowed_tools()` 就是那個「幫你翻譯日常用語成系統正式名稱」的店員。
...
> ⚠️ **初學者常見誤區**：不要以為模型看到的工具清單就是系統實際擁有的全部工具。實際上，模型看到的是經過 `--allowedTools` 過濾、經過 registry 篩選後的「精選清單」。就像餐廳的外送平台菜單可能只有店內菜單的一半——這是刻意設計的，不是 bug。
...
- [ ] 我能說出 `ToolSpec` 包含的四個欄位，以及為什麼要把它們綁在一起
- [ ] 我能區分 registry（有哪些工具可選）和 dispatch（怎麼執行工具）的不同
- [ ] 我能解釋為什麼模型看到的工具清單和系統全部工具不一定相同
- [ ] 我能說明 `normalize_allowed_tools()` 為什麼需要存在
- [ ] 我能區分 built-in、runtime、plugin 三種工具來源的差異
- [ ] 我理解為什麼「能力宣告與治理綁在一起」是 tool system 最核心的設計原則

## 📖 關鍵概念速查表

| 術語 | 說明 |
|------|------|
| **ToolSpec** | 單一工具的正式宣告，包含 name、description、input_schema、required_permission |
| **GlobalToolRegistry** | 集中管理所有工具來源的 registry，負責衝突檢查與統一輸出 |
| **definitions()** | 把 registry 中允許暴露的工具轉成模型可用的 `ToolDefinition` 列表 |
| **normalize_allowed_tools()** | 把使用者輸入的工具名稱（含 alias）正規化為 canonical name |
| **canonical name** | 工具的正式內部名稱，例如 `read_file`、`write_file` |
| **alias** | 工具的常用簡稱，例如 `read`、`write`、`edit` |
| **built-in tools** | 系統內建的工具，由 `mvp_tool_specs()` 定義 |
| **runtime tools** | 在執行期動態加入的工具 |
| **plugin tools** | 來自外部擴充的工具 |
| **tool dispatch** | 當模型發出 tool use 時，把請求導向正確執行器的路由機制 |
| **input_schema** | 工具的輸入格式定義，讓模型知道該傳什麼參數 |
| **required_permission** | 工具需要的最低權限等級 |
...
> ### 💬 本章一句話總結
> 
> **Tool system 不只是「把工具接上去」，而是讓每個工具都有身分證、有門禁卡、有統一管理中心的能力治理體系。**
