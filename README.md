...
| **Vercel 動態版** | 將本目錄連到 Vercel 後，開啟站台的 `/`（`public/index.html`）：透過 **Node** 版 `/api/doc` **執行時讀取** repo 內 `.md`，前端以 Markdown 渲染；**不需把正文寫死在 HTML**。`reader.html` 與 `public/book-manifest.json` 請在**本機**執行 `python3 tools/build_reader.py`（先 `pip install -r tools/requirements.txt`）後 commit／推送——Vercel 上不跑 Python，以免被誤判為 Python 專案而建置失敗。 |

部署提示：在 Vercel 將 **Root Directory** 設為本資料夾；`vercel.json` 僅含 rewrite。專案根目錄有 `package.json`，建置走 **Node**；Serverless 為 `api/doc.js`。
...
- **`public/index.html`（Vercel／本機動態版）**：需可存取 `/api/doc`（部署於 Vercel 時由 Serverless 提供）；章節列表來自 `book-manifest.json`。
- `index.md`：全書目錄與章節入口
- `roadmap.md`：自主學習路線圖
